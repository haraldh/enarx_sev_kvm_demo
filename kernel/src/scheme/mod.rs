...
impl SchemeList {
    ...
    fn new_ns(&mut self) -> SchemeNamespace {
        ...
        self.insert(ns, Box::new(*b"event"), |_| Arc::new(Box::new(EventScheme)))
            .unwrap();
        self.insert(ns, Box::new(*b"itimer"), |_| {
            Arc::new(Box::new(ITimerScheme::new()))
        })
        .unwrap();
        self.insert(ns, Box::new(*b"memory"), |_| {
            Arc::new(Box::new(MemoryScheme::new()))
        })
        .unwrap();
        self.insert(ns, Box::new(*b"sys"), |_| {
            Arc::new(Box::new(SysScheme::new()))
        })
        .unwrap();
    ...
    fn new_root(&mut self) {
        ...
        */
        ...
        .unwrap();
        self.insert(ns, Box::new(*b"proc"), |scheme_id| {
            Arc::new(Box::new(ProcScheme::new(scheme_id)))
        })
        .unwrap();
    ...
    pub fn iter(
        &self,
    ) -> ::alloc::collections::btree_map::Iter<SchemeId, Arc<Box<dyn Scheme + Send + Sync>>> {
        self.map.iter()
    }
    ...
    pub fn get_name(
        ...
        ns: SchemeNamespace,
        name: &[u8],
    ) -> Option<(SchemeId, &Arc<Box<dyn Scheme + Send + Sync>>)> {
    ...
    where
        F: Fn(SchemeId) -> Arc<Box<dyn Scheme + Send + Sync>>,
    {
        if let Some(names) = self.names.get(&ns) {

...
impl GlobalDescriptorTable {
    ...
    pub fn add_entry(&mut self, entry: Descriptor) -> SegmentSelector {
        let mut rpl = PrivilegeLevel::Ring0;
    ...
    pub fn set_offset(&mut self, index: u16, offset: u32) {
        let index = index as usize;
        self.table[index].offsetl = offset as u16;
        self.table[index].offsetm = (offset >> 16) as u8;
        self.table[index].offseth = (offset >> 24) as u8;
    }

    pub fn set_limit(&mut self, index: u16, limit: u32) {
        let index = index as usize;
        self.table[index].limitl = limit as u16;
        self.table[index].flags_limith =
            self.table[index].flags_limith & 0xF0 | ((limit >> 16) as u8) & 0x0F;
    }
    ...
    fn push(&mut self, value: u64) -> usize {
        if self.next_free < self.table.len() {
            ...
            let index_ptr = &mut self.table[index] as *mut _ as *mut u64;
            unsafe { index_ptr.write(value) };
            self.next_free += 1;
            index
        } else {
            panic!("GDT full");

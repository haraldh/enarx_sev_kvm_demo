...
use super::serial::_print;
use super::serial_print;
...
macro_rules! alloc_panic {
    ...
    ($msg:expr) => ({
        $crate::alloc_fmt::panic(&(format_args!($msg), file!(), line!(), column!()))
    });
    ($fmt:expr, $($arg:tt)*) => ({
        $crate::alloc_fmt::panic(&(format_args!($fmt, $($arg)*), file!(), line!(), column!()))
    })
}
...
macro_rules! alloc_assert {
    ...
    ($pred:expr, $fmt:expr, $($arg:tt)*) => ({
        if !($pred) {
            $crate::alloc_panic!(concat!("assertion failed: ", $fmt), $($arg)*);
        }
    })
}
